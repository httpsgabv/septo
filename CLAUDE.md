...
- **e2e**: espere `body[data-hydrated]` (helper `gotoHydrated`) antes de interagir, inclusive depois de `page.reload()`; texto SSR visível não significa que os handlers já estão vivos.
...
- **Fronteira de reminders**: `notes` continua dono de `remindAt` e exporta somente o `ReminderSource` read-only (`listDue`/`findCurrent`). `reminders` não importa o Prisma nem o repositório de notas e nunca limpa/altera a nota depois de enviar.
- **Scheduler de reminders**: tick imediato no boot + intervalo de 60 s, sem sobreposição local; janela de recuperação de 24 h, batch 100, retries imediata/+1 min/+5 min e semântica pelo menos uma vez. Só fica desligado quando `NODE_ENV=test`, para as suítes não enviarem push externo. Antes de múltiplas instâncias, é obrigatório trocar a trava em memória por claim transacional ou fila.
- **Web Push é credencial**: endpoint, `p256dh`, `auth` e `VAPID_PRIVATE_KEY` nunca aparecem em resposta/log. Produção exige `VAPID_PUBLIC_KEY`, `VAPID_PRIVATE_KEY` e `VAPID_SUBJECT` próprios e recusa o par fixo de dev. `404`/`410` remove assinatura; rede, `408`, `429` e `5xx` fazem retry; outros `4xx` são permanentes.
- **Client Push**: use somente o client Orval. Feature detection, sem user-agent; `requestPermission()` só dentro do clique em Ativar. Reload apenas reconcilia assinatura já concedida. Na desativação, DELETE na API vem antes de `unsubscribe()`; falha remota preserva o browser.
- **PWA/SW**: `/sw.js` trata apenas `install`, `activate`, `push` e `notificationclick`. Não adicionar `fetch`, Cache Storage, Workbox ou conteúdo autenticado sem nova spec. O click só navega para URL same-origin; payload inválido cai em `/notes`. Registro ocorre client-only depois da hidratação.
- **Teste real de Push**: E2E controla as browser APIs e não fala com FCM/APNs/Mozilla. Entrega com app fechado, latência, permissão do SO e click real ficam em `tasks/reminders/smoke.md`; plataforma indisponível continua “não executado”, nunca aprovada por inferência.
...
- **Leitura expandida do README** é o próprio `Pane` da leitura ocupando as duas colunas (`md:col-span-2`) com o de markdown `hidden` (continua montado): cobre só a escrita, o resto do layout fica. Não é modal, não é a Fullscreen API nem um segundo Tiptap.
- **Saída do JSON guarda o último resultado válido** enquanto a entrada está quebrada (ajuste de estado durante o render, não `useEffect`); entrada vazia limpa.
