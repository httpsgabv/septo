...
| Erro de JSON | A posição vem da mensagem do motor (`at position N`) e é convertida em linha/coluna contando o texto; sem a posição, mostro a mensagem crua | `JSON.parse` nativo já valida certo; escrever um parser só para dar a posição seria trocar corretude por mensagem bonita. `// ponytail: posição extraída da mensagem do motor; um parser próprio se a mensagem mudar de forma` |
...
| Limite do XML | XML → valor é reconhecidamente com perda: atributos viram chaves `@_attr`, texto misto se perde, tudo vira string | É o comportamento do `fast-xml-parser` e está documentado na UI numa linha. Quem precisa de XML fiel não usa um conversor de bolso |
